		...
		<div class="page-leftheader">
			...
			<ol class="breadcrumb">
				<li class="breadcrumb-item">
					<a href="#">
						<i class="fe fe-server mr-2 fs-14"></i>Status</a>
				</li>
			</ol>
		</div>
                            ...
                            <div class="col-xl-12">
                                <div class="card">
                                    <div class="card-header">
                                        <h3 class="card-title">List Des Status</h3>
                                    </div>
                                    <div class="card-body">
                                                ...
                                                <div class="tab-pane active" id="order_status_tab">
                                                   ...
                                                   <button id="add_order_status" class="btn btn-sm btn-success"><i class="fe fe-plus"></i></button>
                                                           ...
                                                           <div class="table-responsive">
                                                               <table class='table card-table table-vcenter text-nowrap' id='reservation_status_table'>
                                                                   <thead>
                                                                       <tr>
                                                                   ...
                                                                   <tbody>
                                                                                                       @foreach($allreservationstatus as  $item )
                                                                                                           <tr  id="id_val_{{$item->id}}">
                                                                                                                       <input type="hidden" name="id_val" class="id_val" value="{{$item->id}}" />
                                                                         
                                                                           <td><input <?php if($item->id == 72){?> disabled <?php } ?> type="text" value="{{$item->label}}" class="form-control label" name="label"></br>
                                                                           ...
                                                                           <td>
                                                                               ...
                                                                               <span><input <?php if($item->id == 72){?> disabled <?php } ?>  type="color" value="{{$item->color}}" class="form-control color" name="color"></span>
                                                                           </td>
...
<div class="modal-dialog modal-dialog-centered" role="document">
  <div class="modal-content">
    ...
    <form method="post" id="add_status" action="{{route('addnew.resevstatus')}}">
        @csrf
      <input type='hidden' id="type_status" name="type_status" value="RES" />
    
      <div class="modal-body">
        <style style="color:r"></style>
				<div class="row">
...
var url = '{{ route("ajaxdelete.resevstatu",":id") }}';
...
success:function(response){
                ...
                if(response.status == 'success')
                {
                    ...
                    status_new_row_id = "reservation_status_table";
                    status_new_row = $("#reservation_status_table > tbody > tr:first").clone();
                    ...
                    row_html+=' <div id="errorlabel_'+arraydata.id+'" class="error_label text-danger">';
                    row_html+='</div>';
                    row_html+='</td>';
                    row_html+='<td>';
    ...
    $(".update-ordre").on('click', function(e) {
        ...
        console.log($('#type_status').val())
            ...
            $('#reservation_status_table').find('tr').each(function() {
                var id = $(this).attr('id');
                var row = {};
...
success:function(response){
                ...
                if(response.status == 'success')
                {
                    ...
                    $(document).find("#reservation_status_table").css('transition', '3s');
                    $(document).find("#reservation_status_table").css('background-color', '#afa');
                    setTimeout(function() {
                        $(document).find("#reservation_status_table").css('background-color', '#fff');
                    }, 1000);
